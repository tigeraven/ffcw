 ...
 */
function fiddle_form_install_configure_form_alter(&$form, $form_state) {
  // Pre-populate the site name with the server name.
  $form['site_information']['site_name']['#default_value'] = $_SERVER['SERVER_NAME'];
  $form['#submit'][] = 'fiddle_form_submit';
}


function fiddle_form_submit($form, &$form_state) {
  // Import database fiddle file.
  $fiddle_file = dirname(__FILE__) . '/fiddle.sql';
  $success = import_fiddle($fiddle_file);
...
/**
 * Imports a database fiddle file.
 *
 * @see demo_reset().
 */
function import_fiddle($filename) {
  ...
  if (!file_exists($filename) || !($fp = fopen($filename, 'r'))) {
    drupal_set_message(t('Unable to open fiddle file %filename.', array('%filename' => $filename)), 'error');
    return FALSE;
  }
